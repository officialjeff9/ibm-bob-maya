...
Automating 3D graphics pipelines, asset organization, and geometry creation in Autodesk Maya through local AI agent orchestration and Python bridge integration.
...
Viewport Bridge connects modern AI workflows and Model Context Protocol (MCP) servers directly into Autodesk Maya. By establishing a direct communication link between Maya's Python environment and a local AI agent assistant, technical artists can analyze reference inputs, convert conceptual intent into structured data, and automatically drive geometry creation, lighting placement, and set layouts directly inside the Maya viewport.

Architecture & Workflow
AI Agent / Client: Configured as a specialized Technical Artist AI assistant capable of interpreting natural language prompts, scene descriptions, and spatial requirements.

Bridge Layer (bridge.py): Acts as the execution bridge, parsing structured output from the agent persona into native Maya Python (maya.cmds) commands.
...
Plaintext
+-------------------+      +-------------------+      +---------------------+
|  Reference Input  | ---> |   AI Agent/LLM    | ---> |  bridge.py Engine   |
...
Agentic Execution: Translates high-level creative prompts into structured procedural modeling steps and lighting directives that bridge.py executes directly inside Maya.
...
Launch the background file watcher terminal script to start streaming commands into your active Maya session.
